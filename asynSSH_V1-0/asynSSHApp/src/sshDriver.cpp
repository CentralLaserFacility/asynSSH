...
SSHDriverStatus SSHDriver::write(const char *buffer, size_t bufferSize, size_t *bytesWritten, int timeout)
{
  ...
  *bytesWritten = 0;
  ...
  // Small wait to ensure we can read back what was written
  usleep(10000);
 ...
 * @return - Success or failure.
 */
SSHDriverStatus SSHDriver::read(char *buffer, size_t bufferSize, size_t *bytesRead, int timeout)
{
